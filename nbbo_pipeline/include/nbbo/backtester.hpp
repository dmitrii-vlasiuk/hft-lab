...
struct StrategyConfig {
  ...
  // Edge-mode selector:
  //   0 = legacy behavior (no costs, gate on expected_edge_ret > 0)
  //   1 = new behavior, Mode A (costs on, no expected-edge gate: trade-all)
  //   2 = new behavior, Mode B (costs on, absolute expected-edge gate)
  int edge_mode = 2;  // default to "new, Mode B"
};
