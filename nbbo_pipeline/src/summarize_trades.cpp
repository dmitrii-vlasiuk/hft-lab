...
// Schema: ts_in,ts_out,day,mid_in,mid_out,spread_in,
//         direction_score,expected_edge_ret,cost_ret,gross_ret,net_ret,side
//
// We want the 11th field (0-based index 10), i.e. the substring between
// the 10th and 11th commas.
static bool extract_net_ret(const std::string& line, double& out_net_ret) {
  ...
  if (end == std::string::npos) {
    end = line.size();  // net_ret is last field before EOL
  }
...
struct YearStats {
  ...
  double max_gain = -1e300;   // max net_ret (best trade)
  double max_loss =  1e300;   // min net_ret (worst trade)
};
...
int main(int argc, char** argv) {
  ...
  for (int y : years) {
    ...
    double win_pct = (n > 0) ? 100.0 * static_cast<double>(stats.num_wins) / n : 0.0;
    double loss_pct = (n > 0) ? 100.0 * static_cast<double>(stats.num_losses) / n : 0.0;

    double avg_win = (stats.num_wins > 0)
                         ? stats.sum_win_net / static_cast<double>(stats.num_wins)
                         : 0.0;
    double avg_loss = (stats.num_losses > 0)
                          ? stats.sum_loss_net / static_cast<double>(stats.num_losses)
                          : 0.0;  // this will be negative
    ...
    std::cout << std::setw(6) << y << "  "
              ...
              << std::setprecision(8)  // reset precision for next line
              << "\n";
  }
